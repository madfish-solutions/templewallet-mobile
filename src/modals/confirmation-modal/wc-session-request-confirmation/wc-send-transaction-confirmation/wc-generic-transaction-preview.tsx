...
export const WcGenericTransactionPreview: FC<Props> = ({ transaction, chainId, accountAddress }) => {
  ...
  const description = isDefined(transaction.to) ? 'Contract interaction' : 'Contract creation';
  ...
  if (!isDefined(groups) || groups.length === 0) {
    return <OperationPreviewCard iconSeed={transaction.to} description={description} publicKeyHash={transaction.to} />;
  }
  ...
  return (
    <WcBalancesChangesPreviewGroups
      ...
      getPublicKeyHash={group => group.receiver ?? transaction.to}
    />
  );
};
