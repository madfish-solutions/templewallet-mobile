...
it('secureCellSealWithPassphraseEncrypt64 should encrypt a message with a given password', async () => {
  const encryptionOutput = await mockNativeThemis.secureCellSealWithPassphraseEncrypt64(
    mockPassphrase,
    mockUnencryptedString,
    mockSymmetricKey64
  );
...
it('secureCellSealWithPassphraseDecrypt64 should decrypt a message with correct password', async () => {
  const decryptionOutput = await mockNativeThemis.secureCellSealWithPassphraseDecrypt64(
    mockPassphrase,
    mockEncryptedString,
    mockSymmetricKey64
  );
...
it('secureCellSealWithPassphraseDecrypt64 should return undefined if a password is incorrect', async () => {
  await expect(
    mockNativeThemis.secureCellSealWithPassphraseDecrypt64('incorrectPassword', mockEncryptedString, mockSymmetricKey64)
