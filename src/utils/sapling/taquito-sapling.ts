...
import { SaplingTransactionViewer } from './sapling-tx-viewer';
...
export class SaplingToolkit {
  ...
  constructor(
    ...
    saplingTxBuilder = new SaplingTransactionBuilder(keys, saplingForger, saplingContractDetails, readProvider)
  ) {
  ...
  private formatTransactionParams(txParams: ParametersSaplingTransaction[], validateDestination: (to: string) => void) {
    ...
    txParams.forEach(param => {
      ...
      const amountMutez = param.mutez ? param.amount.toString() : format('tz', 'mutez', param.amount).toString();
  ...
  private async getRoot() {
    if (this.#saplingId) {
      ...
      const { root } = await this.#readProvider.getSaplingDiffByContract(this.#contractAddress, 'head');
