...
export class SaplingTransactionBuilder {
  ...
  async createShieldedTx(
    ...
    const { signature, inputs, outputs } = await this.#saplingWrapper.withProvingContext(
      async (saplingContext: string) => {
        const outputs: SaplingTransactionOutput[] = [];
        const inputs: SaplingTransactionInput[] = [];

        for (const i in saplingTransactionParams) {
          const [address] = b58DecodeAndCheckPrefix(saplingTransactionParams[i].to, [PrefixV2.SaplingAddress]);
          outputs.push(
  ...
  async prepareSaplingOutputDescription(
    ...
    const { commitmentValue, commitment, proof } = await this.#saplingWrapper.preparePartialOutputDescription({
    ...
    const ephemeralPublicKey = await this.#saplingWrapper.deriveEphemeralPublicKey(diversifier, ephemeralPrivateKey);
  ...
  async prepareSaplingSpendDescription(saplingContext: string, inputsToSpend: Input[]) {
    ...
    for (let i = 0; i < inputsToSpend.length; i++) {
      ...
      const witness = await this.#saplingState.getWitness(stateTree, new BigNumber(inputsToSpend[i].position));
      ...
      const unsignedSpendDescriptionBytes = this.#saplingForger.forgeUnsignedTxInput(unsignedSpendDescription);
  ...
  async encryptCiphertext(
    ...
    const recipientDiversifiedTransmissionKey = await this.#saplingWrapper.getPkdFromRawPaymentAddress(
    ...
    const payloadOut = Buffer.from(
      secretBox(
        ...
        nonceOut,
        Buffer.concat([recipientDiversifiedTransmissionKey, parametersCiphertext.ephemeralPrivateKey])
      )
    );
