...
import { convertValueToBigNumber, readableFormat } from './helpers';
...
export class SaplingTransactionViewer {
  ...
  private async decryptCiphertextAsReceiver(
    ...
    if (decrypted) {
      ...
      const paymentAddress = bufToUint8Array(
        Buffer.from(await getRawPaymentAddress(toBase64(incomingViewingKey), toBase64(diversifier)), 'base64')
      );
  ...
  private async decryptCiphertextAsSender(
    ...
    const outgoingCipherKey = blake.blake2b(
      bufToUint8Array(Buffer.from(concat, 'hex')),
      bufToUint8Array(Buffer.from(OCK_KEY)),
      32
    );
  ...
  private async decryptCiphertext(keyAgreementHash: Uint8Array, nonce: Uint8Array, payload: Uint8Array) {
  ...
  private async isSpent(
    ...
    const computedNullifier = await computeNullifier(
      toBase64(this.#viewingKeyProvider.getFullViewingKey()),
      toBase64(address),
      Number(value),
      toBase64(randomCommitmentTrapdoor),
      position
    );

    return nullifiers.includes(Buffer.from(computedNullifier, 'base64').toString('hex'));
  }
}
