...
function normalizeAddresses(values: unknown) {
  if (!Array.isArray(values)) {
    throw new WcEvmRequestError('invalid-params', 'Invalid addresses');
  }
...
function normalizeIntegers(type: string, values: unknown) {
  if (!Array.isArray(values)) {
    throw new WcEvmRequestError('invalid-params', 'Invalid integers');
  }
...
export function typedV1SignatureHash(typedData: OldTypedDataField[]) {
  ...
  const types = normalizedData.map(e => {
    if (e.type === 'function') {
      throw new WcEvmRequestError('invalid-params', 'Unsupported or invalid type: "function"');
    }
  ...
  const schema = typedData.map(e => {
    if (!e.name) {
      throw new WcEvmRequestError('invalid-params', `Invalid name in ${JSON.stringify(e)}`);
    }
