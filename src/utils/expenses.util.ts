...
export function tryParseExpenses(operations: OperationsPreview, accountAddress: string): RawOperationExpenses[] {
  ...
  return (Array.isArray(operationsAsArray) ? operationsAsArray : [])
    .map<RawOperationExpenses | undefined>(operation => {
      ...
      const parsedAmount = amount !== undefined ? new BigNumber(amount) : undefined;
      if (!kind) {
        return undefined;
      }
      if (kind === 'delegation') {
